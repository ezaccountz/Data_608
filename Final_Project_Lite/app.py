...
#
#Data is loaded using the Socrata Open Data API 
##############################################################################

#settings for Socrata API
app_token = 'HcYUIkQM18kU32TiVXYsAKVjy'
address = 'https://data.cityofnewyork.us/resource/h9gi-nx95.json'
#we will load the year, month, time, latitude, longtidue, collision factor, vehicle type from the database
...
#Create a dictionary to store dataframes. The collision data for each calendar year is loaded in a dataframe.
#Once a dataframe for a specific year is loaded, it is cleaned and stored in the dict for later use.
dfs = dict()
...
#Load the distinct years from the database, they will be the value of a dropdown list that allows the user to select a year
...
#Load the distinct collision factors from the database
url = address + '?$select=distinct(contributing_factor_vehicle_1)%20as%20factors'
url = url + "&$limit="+str(limit)
url = url + "&$$app_token="+app_token
factors = pd.read_json(url)
#Convert the distinct original factors into distinct revised factors, using the above loaded factor table
...
#Load the distinct vehicle types from the database. 
#The vehicle type is a non-categorical variable and it is case-sensitive in the database.
#Some of them contains unicode characters. It requires more work on cleaning up the data. 
url = address + '?$select=distinct(vehicle_type_code1)%20as%20vehicle_types'
url = url + "&$limit="+str(limit)
url = url + "&$$app_token="+app_token
vehicle_types = pd.read_json(url)
#Convert the vehicle types into upper case
vehicle_types['vehicle_types'] = vehicle_types['vehicle_types'].str.upper()
#Replace na with 'Unspecified', which is one of the original vehicle types
vehicle_types['vehicle_types'].fillna('Unspecified', inplace = True)
#Remove non-word characters, which can also remove the unicode characters
vehicle_types['vehicle_types']  = vehicle_types['vehicle_types'].map(lambda x: re.sub(r'\W+', ' ', x))
#Trim the vehicle types
vehicle_types['vehicle_types'] = vehicle_types['vehicle_types'].str.strip()
#Convert the distinct original vehicle types into distinct revised vehicle types, using the above loaded factor table
...
def getDF(selected_year):
    #Check if the dataframe for a specific year in the dfs dict, if not, load the dataframe from the database.
    if not (selected_year in dfs):
        #Load the dataframe ftom the database
        url = address
        ...
        #Convert the original factors into revised factors using the above loaded factor table
        dfs[selected_year]['factor'].replace(factor_cov, inplace = True)
        #Replace na with 'Unspecified' for the factors
        ...
        #Convert the original vehicle types into revised vehicle types using the above loaded factor table
        dfs[selected_year]['vehicle_type'].replace(vehicle_type_cov, inplace = True)
        #drop the records with na values        
        dfs[selected_year].dropna(inplace = True)
    #finally return the dataframe for the specific year 
...
#Load the dataframe of the most recent year. It will be used to set up our initial Dash page
getDF(years[0])
